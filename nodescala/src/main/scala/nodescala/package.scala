...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
     ...
     */
    def always[T](value: T): Future[T] = Promise().complete(Try(value)).future
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      def go(fs: List[Future[T]], acc: List[T]): Unit = fs match {
        case Nil => p.complete(Try(acc))
        case (f::fs) => {
          f.onComplete(result => result match {
            case Success(value) => go(fs, acc ++ List(value))
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      f(cts.cancellationToken) onComplete {
        case _ => ()
      }
      cts
    }
